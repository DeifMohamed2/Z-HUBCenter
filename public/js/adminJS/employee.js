...
const getEmployees = async () => {
  try {
    ...
    employees.forEach((employee) => {
      ...
      const toggleButton = employee.isActive 
        ? `<button class="btn btn-danger btn-sm toggle-status-btn" data-id="${employee._id}" data-status="${employee.isActive}">
             <i class="material-symbols-rounded">block</i>
             Deactivate
           </button>`
        : `<button class="btn btn-success btn-sm toggle-status-btn" data-id="${employee._id}" data-status="${employee.isActive}">
    ...
    // Attach event listener for Toggle Status buttons
    document
      .querySelectorAll('.toggle-status-btn')
      .forEach((btn) =>
        btn.addEventListener('click', (e) => toggleEmployeeStatus(e.currentTarget.dataset.id))
      );
...
const toggleEmployeeStatus = async (id) => {
  try {
    const response = await fetch(`/admin/toggle-employee-status/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    alert('Failed to toggle employee status. Please try again.');
  }
};
