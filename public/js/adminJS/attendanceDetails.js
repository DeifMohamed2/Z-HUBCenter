...
const removedTBody = document.getElementById('removedTBody');
...
async function loadAttendanceDetails() {
  try {
    ...
    // Populate removed students table
    populateRemovedStudentsTable(data.studentsDeleted || []);
...
// Populate removed students table
function populateRemovedStudentsTable(removedStudents) {
  removedTBody.innerHTML = '';

  if (!removedStudents || removedStudents.length === 0) {
    removedTBody.innerHTML = `
      <tr>
        <td colspan="9" class="text-center text-muted">No removed students</td>
      </tr>
    `;
    return;
  }

  removedStudents.forEach((entry, index) => {
    const row = document.createElement('tr');
    row.innerHTML = `
      <td class="text-center">${index + 1}</td>
      <td class="text-center">${entry.student?.studentName || 'Unknown'}</td>
      <td class="text-center">${entry.student?.studentCode || 'N/A'}</td>
      <td class="text-center" dir="ltr">${formatCurrency(entry.amountPaid)}</td>
      <td class="text-center" dir="ltr">${formatCurrency(entry.feesApplied)}</td>
      <td class="text-center">${entry.addedBy?.employeeName || 'Unknown'}</td>
      <td class="text-center">${entry.deletedBy?.employeeName || 'Unknown'}</td>
      <td class="text-center">${entry.reason || '—'}</td>
      <td class="text-center">${entry.deletedAt ? formatDate(entry.deletedAt) : '—'}</td>
    `;
    removedTBody.appendChild(row);
  });
}
