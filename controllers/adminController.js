...
const getDashboardAnalytics = async (startDate, endDate) => {
  try {
    ...
    for (const employee of employees) {
      // Get employee's KPIs
      const employeeKPIList = employeeKPIs.filter(kpi => kpi.employee && kpi.employee._id.toString() === employee._id.toString());
...
const getAttendanceDetails = async (req, res) => {
  try {
    ...
    const attendance = await Attendance.findById(id)
      ...
      .populate('studentsPresent.addedBy', 'employeeName')
      .populate('studentsDeleted.student', 'studentName studentCode studentPhoneNumber studentParentPhone')
...
const getTeacherReportData = async (req, res) => {
  try {
    ...
    attendanceRecords.forEach(record => {
      ...
      // Collect removed students data
      if (Array.isArray(record.studentsDeleted) && record.studentsDeleted.length > 0) {
        record.studentsDeleted.forEach(removed => {
          studentsData.push({
            sessionId: record._id,
            sessionDate: record.createdAt,
            studentName: removed.student?.studentName || 'Unknown',
            studentCode: removed.student?.studentCode || 'N/A',
            studentPhone: removed.student?.studentPhoneNumber || 'N/A',
            parentPhone: removed.student?.studentParentPhone || 'N/A',
            amountPaid: removed.amountPaid,
            centerFees: removed.feesApplied || 0,
            addedBy: removed.addedBy?.employeeName || 'Unknown',
            deletedBy: removed.deletedBy?.employeeName || 'Unknown',
